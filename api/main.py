...
async def health():
    ...
    async def _check_hubspot() -> str:
        try:
            import os
            from core.hubspot_client import get_pending_leads
            if not os.environ.get("HUBSPOT_ACCESS_TOKEN", ""):
                return "error"
            await get_pending_leads()
            return "ok"
        except Exception as exc:
            logger.warning("Health check HubSpot failed: %s", exc)
            return "error"
    ...
    results = await asyncio.gather(
        ...
        asyncio.wait_for(_check_hubspot(),  timeout=20),
        return_exceptions=True,
    )

    labels = ["redis", "supabase", "evolution", "hubspot"]
...
async def _run_hunter_batch(request: HunterBatchRequest, checkpointer: Any) -> None:
    """
    Background task: load leads from HubSpot and run the HUNTER graph.
